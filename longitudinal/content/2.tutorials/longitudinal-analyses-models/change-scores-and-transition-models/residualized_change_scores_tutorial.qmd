...
Residualized change scores are a useful statistical approach for measuring change over time when you are interested in isolating the unique variance of an outcome at a later time point after accounting for baseline levels. This method helps control for individual differences at baseline and allows for a more precise estimate of the relationship between predictors and change in the outcome. Residualized change scores are typically calculated by regressing a follow-up score (e.g., Time 2) on a baseline score (e.g., Time 1) and extracting the residuals. These residuals represent the amount of change unexplained by the baseline score.
