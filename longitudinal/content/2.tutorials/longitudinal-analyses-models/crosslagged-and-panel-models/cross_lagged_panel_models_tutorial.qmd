...
Cross-lagged panel models (CLPMs) are used to examine the reciprocal relationships between two or more variables over time. They allow you to assess both the stability of each variable and the cross-lagged effects, which indicate whether changes in one variable predict subsequent changes in another. CLPMs are particularly useful when studying the dynamic interplay between variables, such as the relationship between academic achievement and motivation over time.

By including both autoregressive (within-variable) and cross-lagged (between-variable) paths, CLPMs offer a detailed view of how variables influence each other over multiple time points.
