...
Multivariate Latent Growth Curve Models (MLGCMs) are an extension of latent growth models that allow for the simultaneous modeling of change in multiple variables over time. MLGCMs not only capture the individual growth trajectories for each variable but also allow for the examination of how these trajectories are related to each other. This method is useful when you have multiple outcomes that change over time and you want to understand their shared growth dynamics.

By modeling multiple outcomes together, MLGCMs provide insights into the correlations between the growth factors (intercepts and slopes) of different variables.
