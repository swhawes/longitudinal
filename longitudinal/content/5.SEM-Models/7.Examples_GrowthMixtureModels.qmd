...
#######
## Windsorizing the weight variable
# Set the quantile thresholds for winsorizing
lower_quantile <- 0.05
upper_quantile <- 0.95

# Calculate the quantile thresholds
quantiles <- quantile(df_long$weight, probs = c(lower_quantile, upper_quantile), na.rm = TRUE)
lower_bound <- quantiles[1]
upper_bound <- quantiles[2]

# Winsorize the weight variable
df_long$weight_adjusted <- ifelse(df_long$weight < lower_bound, lower_bound, 
                                  ifelse(df_long$weight > upper_bound, upper_bound, df_long$weight))

# Check the summary of the adjusted weight variable
summary(df_long$weight_adjusted)
...
gmm2 <- gridsearch(rep = 50, maxiter = 5, minit = gmm1,
hlme(weight_adjusted ~ event, subject = "id_numeric", random=~1,
